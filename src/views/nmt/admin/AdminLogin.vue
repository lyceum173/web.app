<template>
    ...
    import { auth, signInWithRedirect, provider } from '@/assets/js/app';
    // setTimeout(() => {
    //     authed.value = true
    // }, 5000)
    ...
    const authLogin = () => {
        ...
        setTimeout(() => {
            signInWithRedirect(auth, provider).then(u => {
            if (!u.user.email.endsWith("@lyceum173.kyiv.ua")) {
                error.value = "Доступ обмежений до учні вта вчителів Ліцею №173"
            }
