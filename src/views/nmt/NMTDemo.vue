        ...
        <div class="test-content">
          ...
          <main class="test">
            <div class="sequence-nav">
              <button class="sequence-nav-button button-previous" :class="{disabled: activeTestIndex === 0}" :disabled="activeTestIndex === 0" @click="setActiveBlock(0)">
                <span>
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" class="injected-svg" data-src="https://cdn.hugeicons.com/icons/arrow-right-01-stroke-standard.svg" xmlns:xlink="http://www.w3.org/1999/xlink" role="img" color="#000000">
<path d="M9.00005 6L15 12L9 18" stroke="#000000" stroke-width="1.5" stroke-miterlimit="16" stroke-linecap="round" stroke-linejoin="round"></path>
              ...
              <button class="sequence-nav-button button-next" :class="{disabled: activeTestIndex === 1}" :disabled="activeTestIndex === 1" @click="setActiveBlock(1)">
                <span>
                  <span class="sequence-nav-button-label">Вперед</span> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="" viewBox="0 0 24 24" fill="none" class="injected-svg" data-src="https://cdn.hugeicons.com/icons/arrow-right-01-stroke-standard.svg" xmlns:xlink="http://www.w3.org/1999/xlink" role="img" color="#000000">
<path d="M9.00005 6L15 12L9 18" stroke="#000000" stroke-width="1.5" stroke-miterlimit="16" stroke-linecap="round" stroke-linejoin="round"></path>
</svg>
            ...
            <br>
            <div class="question" v-for="(question, qIndex) in currentBlockQuestions"
              :key="`${activeTestIndex}-${qIndex}`" :data-q="qIndex">
              ...
              <SingleAnswerComponent v-if="question.t === 's'" :data="question.a" :savedAnswers="question.s"
                :v-model="question.current" @update="match => handleSingleUpdate(match, qIndex)" />

              <MultipleAnswersComponent v-else-if="question.t === 'm'" :data="question.a"
                :savedAnswers="question.s ? [...question.s] : []" :v-model="question.current"
                @save="result => handleSave(result, qIndex)" @update="matches => handleMultiUpdate(matches, qIndex)" />
              ...
              <ShortAnswerComponent v-else-if="question.t === 'sh'" :data="question.a" :savedAnswers="question.s"
                :v-model="question.current" @update="match => handleSingleUpdate(match, qIndex)" />
                
              <button v-if="question.t === 'st'" class="submit" :disabled="validSet(question)"
                @click="saveAnswers(qIndex)">
                Зберегти відповідь
              </button>
              ...
              <div v-if="question.saved" class="answer-saved">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none"
                  class="injected-svg">
                  <path fill-rule="evenodd" clip-rule="evenodd"
                    d="M1.25 12C1.25 6.06294 6.06294 1.25 12 1.25C17.937 1.25 22.75 6.06293 22.75 12C22.75 17.937 17.937 22.75 12 22.75C6.06293 22.75 1.25 17.937 1.25 12ZM12.1949 10.0309C12.4605 10.0702 12.8087 10.1752 13.0935 10.4884C13.3659 10.7881 13.4444 11.1308 13.4743 11.3752C13.5002 11.5871 13.5001 11.8365 13.5 12.0602L13.5 12.1V16.5C13.5 17.0523 13.0523 17.5 12.5 17.5C11.9477 17.5 11.5 17.0523 11.5 16.5V12.1L11.5 12C10.9477 12 10.5 11.5523 10.5 11C10.5 10.4477 10.9477 10 11.5 10L11.5408 9.99999H11.5408C11.739 9.99983 11.9835 9.99964 12.1949 10.0309ZM12 7C11.4477 7 11 7.44772 11 8C11 8.55228 11.4477 9 12 9H12.0001C12.5524 9 13.0001 8.55228 13.0001 8C13.0001 7.44772 12.5524 7 12.0001 7H12Z"
                    fill="var(--nmt)">
                  </path>
                </svg>
...
<path d="M9.00005 6L15 12L9 18" stroke="#000000" stroke-width="1.5" stroke-miterlimit="16" stroke-linecap="round" stroke-linejoin="round"></path>
        ...
        <div class="test-nav">
          ...
          <div class="side-navigation-items js-task-list">
            ...
            <a class="task-block" v-for="(question, index) in currentBlockQuestions" :key="index"
              @click="scrollTo(index)"
              :class="{ active: currentQuestion === index, 'submitted': question.saved === true }">
              <span>{{ index + 1 }}</span>
    ...
    <div class="holder">
      <div class="footer-holder">
        <div class="partner-item">
          <img src="https://lyceum173.kyiv.ua/assets/img/template/logo_small.webp" alt="" @dragstart.prevent style="width: 100px">
        </div>
      </div>
    </div>
  </footer>
...
const examTitle = ref("Demo")
const examBlockTitle = ref("Demo - Block 1")
const STORAGE_KEY = 'saved-questions'
const blockLoaded = ref(false)
const questions = ref([
  [
    { t: "s", q: "Яке з чисел є простим?", a: ["4", "6", "7", "9"] },
    { t: "m", q: "Виберіть всі парні числа:", a: ["3", "8", "12", "7"] },
    { t: "st", q: "Співвіднесіть формулу з її описом:", a: [["a² + b² = c²", "F = ma", "V = IR", "E = mc²"], ["Закон Піфагора", "Другий закон Ньютона", "Закон Ома", "Рівняння Ейнштейна"]] },
    { t: "sh", q: "Скільки буде 12 * 12?", a: [] },
    { t: "s", q: "Чому дорівнює 5 + 7?", a: ["10", "11", "12", "13"] },
    { t: "m", q: "Позначте кратні числу 3:", a: ["3", "6", "9", "10"] },
    { t: "sh", q: "Скільки буде 8 * 9?", a: [] },
    { t: "st", q: "Співвіднесіть математиків з їх відкриттями:", a: [["Евклід", "Піфагор", "Архімед", "Ньютон"], ["Геометрія", "Теорема", "Закон важеля", "Гравітація"]] },
    { t: "s", q: "Яке число є квадратом іншого?", a: ["5", "7", "16", "13"] },
    { t: "m", q: "Виберіть прості числа:", a: ["2", "4", "5", "9"] },
    { t: "sh", q: "Скільки буде 15 - 7?", a: [] },
    { t: "st", q: "Зіставте поняття з означеннями:", a: [["Периметр", "Площа", "Кут", "Радіус"], ["Сума сторін", "Кількість поверхні", "Міра повороту", "Відстань до центру"]] },
    { t: "s", q: "Який результат 3²?", a: ["6", "9", "12", "3"] },
    { t: "m", q: "Які числа діляться на 2?", a: ["4", "5", "6", "9"] },
    { t: "sh", q: "Скільки буде 100 / 4?", a: [] },
    { t: "s", q: "Скільки сторін має трикутник?", a: ["2", "3", "4", "5"] },
    { t: "m", q: "Виберіть дроби:", a: ["1/2", "2", "3/4", "5"] },
    { t: "sh", q: "Скільки градусів у прямому куті?", a: [] },
    { t: "s", q: "Яке з чисел є парним?", a: ["3", "5", "8", "7"] },
    { t: "sh", q: "Скільки буде 2³?", a: [] }
  ],
  [
    { t: "s", q: "Яке слово є іменником?", a: ["бігати", "зелений", "дитина", "швидко"] },
    { t: "m", q: "Оберіть дієслова:", a: ["читати", "співати", "веселий", "думка"] },
    { t: "st", q: "Зіставте слово з його частиною мови:", a: [["стіл", "бігти", "гарний", "швидко"], ["іменник", "дієслово", "прикметник", "прислівник"]] },
    { t: "sh", q: "Скільки букв у слові 'книга'?", a: [] },
    { t: "s", q: "Що з наведеного є прикметником?", a: ["плакати", "червоний", "дерево", "швидко"] },
    { t: "m", q: "Виберіть іменники:", a: ["стіл", "біг", "летіти", "книга"] },
    { t: "sh", q: "Скільки складів у слові 'молоко'?", a: [] },
    { t: "st", q: "Зіставте слова з антонімами:", a: [["великий", "світло", "новий", "веселий"], ["маленький", "темрява", "старий", "сумний"]] },
    { t: "s", q: "Яке слово є прислівником?", a: ["повільно", "книга", "зелений", "ходити"] },
    { t: "m", q: "Оберіть прикметники:", a: ["гарний", "хороший", "плавати", "будинок"] },
    { t: "sh", q: "Скільки букв у слові 'словник'?", a: [] },
    { t: "s", q: "Що з наведеного є дієсловом?", a: ["їсти", "м'яч", "високий", "швидкий"] },
    { t: "m", q: "Позначте прислівники:", a: ["швидко", "повільно", "весело", "гора"] },
    // { t: "st", q: "Зіставте рід іменника:", a: [["стілець", "дівчина", "вікно", "рука"], ["чоловічий", "жіночий", "середній", "жіночий"]] },
    { t: "sh", q: "Скільки звуків у слові 'юнак'?", a: [] },
    { t: "s", q: "Що з наведеного є прикметником?", a: ["біг", "зелений", "писати", "швидко"] },
    { t: "m", q: "Оберіть слова на літеру 'п':", a: ["птах", "папір", "вода", "плавати"] },
    { t: "sh", q: "Скільки букв у слові 'учень'?", a: [] },
    { t: "st", q: "Зіставте слова з правильним наголосом:", a: [["книга́", "ру́чка", "молоко́", "дере́во"], ["a", "b", "c", "d"]] },
    { t: "s", q: "Яке слово є іменником?", a: ["малювати", "біг", "зелений", "швидко"] }
  ]
])
...
window.addEventListener("keydown", (e) => {
  if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === "d") {
    ...
    if (!devMode.value) {
      ...
      // Optional: ask confirmation to turn off dev mode
      if (confirm("Turn off Dev Mode?")) {
        devMode.value = false;
...
// Setup interval and load on mount
let observer;

useHead({
  title: "НМТ | Іспит"
})
function loadCurrentBlockQuestions() {
  const block = questions.value[activeTestIndex.value]
  if (!block) {
    console.warn('No questions block found at index', activeTestIndex.value)
    currentBlockQuestions.value = []
    return
  }
  currentBlockQuestions.value = block.map(q => ({
    ...q,
    current: q.savedAnswer || null,
    saved: !!q.savedAnswer,
    savedAnswer: q.savedAnswer || null,
    savedTime: q.savedTime || null
  }))
}
function validSet(q) {
...
async function setActiveBlock(blockIndex) {
  ...
  // Wait for DOM update before querying elements
  await nextTick();
...
function saveAnswers(index) {
  const question = currentBlockQuestions.value[index];
  if (!question) return;

  console.log("Saving answer...");

  const globalQuestion = questions.value[activeTestIndex.value][index];
  let savedAnswer = "";

  // Case: short answer (text input)
  if (question.t === "sh") {
  ...
  // Update global state
  globalQuestion.savedAnswer = savedAnswer;
  globalQuestion.savedTime = new Date();
  globalQuestion.s = savedAnswer;
...
function handleUpdate(matches, index) {
  console.log(matches)
  ...
  const answerList = Array.isArray(question.a?.[1]) ? question.a[1] : []

  const userAnswer = {}

  matches.forEach((answer, i) => {
    if (answer !== null && Array.isArray(answerList)) {
      userAnswer[i] = answerList.findIndex(a => a === answer)
    }
  })

  question.current = userAnswer

  const allAnswered = matches.every(m => m !== null)
...
function handleSetUpdate(match, index) {
  // console.log(questions.value[activeTestIndex][index])
  console.log(currentBlockQuestions.value[index].current)
  const question = currentBlockQuestions.value[index]
  if (!question) {
    console.warn(`Question at index ${index} not found`)
    return
  ...
  const isAnswered = match !== null && match !== ''
  const isSameAsSaved = question.savedAnswer === match
...
onMounted(() => {
  ...
  // Initially observe elements on mount
  document.querySelectorAll('[data-q]').forEach(el => observer.observe(el));
});
...
.question:has(.answer-saved) .short-answer input {
...
.test-nav {
  ...
  /* Adjust based on your layout, e.g., header height */
  flex: 1 0 190px;
  width: auto;
  max-width: 190px;
  ...
  /* Optional: keeps it scrollable within viewport */
  overflow-y: auto;
  background-color: white;
  /* Helps when scrolling under content */
  z-index: 1000;
...
.side-navigation-items a {
  ...
  border-top-color: rgb(197, 203, 214);
  border-right-color: rgb(197, 203, 214);
  border-bottom-color: rgb(197, 203, 214);
  border-left-color: rgb(197, 203, 214);
  background: #f2f6f8;
  color: #000;
  display: flex !important;
...
.side-navigation-items a.submitted {
  background: #e62050 !important;
  color: #fff !important;
  border-color: #e62050;
}

pre {
  background: #E620513E !important;
...
.side-navigation-items a:not(.submitted):hover {
  text-decoration: none !important;
  border-color: #e62050;
  color: #e62050 !important;
  cursor: pointer;
  background: rgba(230, 32, 80, 0.14);
}

.side-navigation-items a.submitted.active {
...
.sequence-nav button.active {
  border-bottom: 3px solid #eb4e74;
...
.sequence-nav button:focus:active {
  box-shadow: none;
  border-color: #e7e7e7;
  border-bottom: 3px solid #eb4e74;
  background-color: #fffefe;
...
.sequence-nav button:not(.active, :disabled):hover {
  border-bottom: 3px solid #eb4e74 !important;
  background-color: #fffefe;
  color: #000000FF;
  border-color: #e7e7e7;
  border-radius: 0px !important
...
.sequence-nav button:not(.active, :disabled):hover span.tab-text {
  position: absolute;
  transform: translate(-50%, calc(-50% + 1.2px)) !important;
}
.sequence-nav button:disabled:hover  {
  /* position: absolute; */
  cursor: not-allowed !important;
  /* transform: translate(-50%, calc(-50% + 1.2px)) !important; */
}
.sequence-nav button:disabled:hover span.tab-text {
  position: absolute !important;
  transform: translate(-50%, calc(-50% + 1.2px)) !important;
}
.sequence-nav button.active span {
  position: absolute;
  transform: translate(-50%, calc(-50% + 1.2px)) !important;
}
...
.sequence-nav-button.button-previous svg {
  transform: rotate(180deg);;
}

.sequence-nav-button > span {
  position: absolute;
  top: 50%;
  left: 50%;
  display: flex;
...
.sequence-nav-button:hover > span svg {
  & * {
    stroke: #000000FF 
  }
}
.sequence-nav-button:hover > span {
